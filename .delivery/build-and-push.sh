...
if [ "$server_changed" = true ]; then
  ...
  cd ${REPO_ROOT}/server
  docker build -t "${DOCKER_HOST}/${DOCKER_TEAM}/zally:${CDP_BUILD_VERSION}" .
  docker push "${DOCKER_HOST}/${DOCKER_TEAM}/zally:${CDP_BUILD_VERSION}"
...
if [ "$ghe_integration_changed" = true ]; then
  ...
  docker build -t "${DOCKER_HOST}/${DOCKER_TEAM}/zally-github-integration:${CDP_BUILD_VERSION}" .
  docker push "${DOCKER_HOST}/${DOCKER_TEAM}/zally-github-integration:${CDP_BUILD_VERSION}"
