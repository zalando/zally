...
class DefineOAuthScopesRuleTest {
    ...
    @Test
    fun emptyAPI() {
        assertThat(rule.defineOAuthScopes(Swagger())).isNull()
    }

    @Test
    fun apiWithoutScope() {
        val swagger = getFixture("api_without_scopes_defined.yaml")
        assertThat(rule.defineOAuthScopes(swagger)!!.paths).hasSize(4)
    }
    ...
    fun apiWithUndefinedScope() {
        ...
        assertThat(rule.defineOAuthScopes(swagger)!!.paths).hasSize(2)
    }
